...
Application::Application()
{
	...
	float* image_data = stbi_loadf("res/night.hdr", &image_width, &image_height, &image_channels, 0);
	if (!image_data) {
		throw std::runtime_error("Failed to load texture image!");
	}
	uint64_t image_size = image_width * image_height * sizeof(float) * image_channels;

	ConvertEquirectangularToCubemap(image_data, image_width, image_height, 1500, pixels);

	createImageDesc(device, cube_texture, vk::Format::eR32G32B32A32Sfloat, 1500, 1500, vk::SampleCountFlagBits::e1, cubemap_usage_flags, vk::ImageAspectFlagBits::eColor, image_type_2d, view_type_cube, 6, vk::ImageCreateFlagBits::eCubeCompatible, device_desc, 1);
